...
public class DoanhThuController implements Initializable {
    ...
    private boolean isThongKeThanhCong;
    private Integer namBaoCao;

    private List<BaoCaoNam> listBaoCaoNam;

    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {
        isThongKeThanhCong = false;
        ...
        dtNam_inBaoCao_btn.setOnMouseClicked(event -> InBaoCao());
    }

    public void InBaoCao() {
        if (isThongKeThanhCong == false) {
            alert.errorMessage("Vui lòng thống kê doanh thu trước khi xuất báo cáo!");
            return;
        }
        reportController.PrintReportBaoCaoNam(namBaoCao, listBaoCaoNam);
    }
    void DTNam_LoadData() {
        ...
        try {
            //set doanh thu nam ve 0
            tongDoanhThuNam = BigDecimal.valueOf(0.0);
            //khoi tao list de in report
            namBaoCao = dtNam_cbbox_namSelection.getSelectionModel().getSelectedItem();
            ...
            while (result.next()) {
                ...
                listBaoCaoNam.add(bcNam);
            }

            dtNam_tongdt_txtfld.setText(tongDoanhThuNam.toString() + " VNĐ");
            //
            isThongKeThanhCong = true;
            //quy dinh cac cot se hien thi thuoc tinh nao
